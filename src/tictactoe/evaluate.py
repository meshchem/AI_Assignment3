...
minimax    = MinimaxAgent(player1=ai_id, opp_player=human_id, use_alpha_beta=False)
minimax_ab = MinimaxAgent(player1=ai_id, opp_player=human_id, use_alpha_beta=True)

qlearn = QLearningAgent(player1=ai_id, opp_player=human_id)
qlearn.load("results/qlearn_ttt.pkl")

dqn = DQNAgent(player1=ai_id, opp_player=human_id)
dqn.load("results/dqn_rule_ttt.pt")

default    = DefaultOpponent(player1=human_id, player2=ai_id)
...
def play_games(agent1, agent2, num_games=NUM_GAMES, label=""):
    ...
    for i in range(num_games):
        ...
        if a1_role == "p1":
            games_as_p1 += 1
            if winner == a1_piece: wins_as_p1 += 1
        else:
            games_as_p2 += 1
            if winner == a1_piece: wins_as_p2 += 1
...
print(f"\n{'-'*60}")
print(f"  Tic Tac Toe: Overall Summary")
print(f"{'-'*60}")

wr_vs_default = {n: vs_default[n]["win"] for n in algo_names}
h2h_wins = {n: [] for n in algo_names}

for label, r in h2h.items():
    h2h_wins[r["name1"]].append(r["win"])
    h2h_wins[r["name2"]].append(r["loss"])

avg_h2h = {n: sum(h2h_wins[n]) / len(h2h_wins[n]) for n in algo_names}
overall  = {n: (wr_vs_default[n] + avg_h2h[n]) / 2  for n in algo_names}

print(f"\n  {'Algorithm':12s}  {'vs Default':>12s}  {'Avg H2H':>10s}  {'Overall':>10s}")
print(f"  {'-'*50}")

for n in algo_names:
    print(f"  {n:12s}  {wr_vs_default[n]:>11.1f}%  "
          f"{avg_h2h[n]:>9.1f}%  {overall[n]:>9.1f}%")
...
# CSV 2: Head-to-Head
with open(f"{OUTPUT_DIR}ttt_h2h.csv", "w", newline="") as f:
    writer = csv.writer(f)
    ...
    for label, r in h2h.items():
        writer.writerow([label, r["name1"], f"{r['win']:.1f}", f"{r['draw']:.1f}",
                         f"{r['loss']:.1f}", f"{r['win_pct_as_p1']:.1f}",
                         f"{r['win_pct_as_p2']:.1f}"])
print(f"Saved to {OUTPUT_DIR}ttt_h2h.csv")

# CSV 3: Overall Summary
with open(f"{OUTPUT_DIR}ttt_overall.csv", "w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(["Algorithm", "vsDefault%", "AvgH2H%", "Overall%"])
    for n in algo_names:
        writer.writerow([n, f"{wr_vs_default[n]:.1f}",
                         f"{avg_h2h[n]:.1f}", f"{overall[n]:.1f}"])
print(f"Saved to {OUTPUT_DIR}ttt_overall.csv")